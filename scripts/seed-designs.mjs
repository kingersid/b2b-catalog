...
// Seed D1 designs table with the 51 existing images + upload them to R2
// Run: node scripts/seed-designs.mjs

const ACCOUNT_ID = 'e80e472d0cd0037855bc396a3b7f7d97';
const DB_ID = 'c1978fb2-5f08-48f9-94b9-58d6bdfc1628';
...
fs.writeFileSync('/tmp/seed-designs.sql', statements);
console.log('SQL written to /tmp/seed-designs.sql');
console.log(`Will upsert ${FILES.length} designs into D1.`);

// 2. Upload original images to R2
...
for (const f of FILES) {
  ...
  const contentType = ext === 'jpg' || ext === 'jpeg' ? 'image/jpeg' : ext === 'webp' ? 'image/webp' : 'image/jpeg';
  
  // Upload via Cloudflare API
  // We'll do this via the Worker proxy after deploy, or use r2.put via a script
  // For now, just log what needs to be uploaded
  uploaded++;
}

console.log(`\n${uploaded} images found locally, ${missing} missing.`);
console.log('After deploy, run: wrangler r2 object put chandni-catalog-assets/designs/original/<id>.jpg --file=<path>');
